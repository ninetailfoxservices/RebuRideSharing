...
void administrator(fstream& driverReg, fstream& customerReg, fstream& bookingInfo);
//void newCustomer();
...
void administrator(fstream& driverReg, fstream& customerReg, fstream& bookingInfo)
{
    // administrator function for calling admin login/admin menu
    int choice;
...
void adminMenu(fstream& driverReg, fstream& customerReg, fstream& bookingInfo)
{
    // admin menu, for displaying daily reports, weekly reports, registered customers and drivers, and all cancelled bookings 
    int choice;

    // function call for admin login
    adminLogin();
...
{
    // admin login screen
    // variable calls for admin login, with universal username and password
    string adminUser, adminPass;
    int attempts = 1;

    printLine();

    // while loop for checking if failed login attempted more than 3 times
    while (attempts < 4)
    {
        cout << "Enter username: ";
        cin >> adminUser;
        ...
        // checking if wrong username or password, less than maximum allowed 3 times
        if (adminUser != "Admin" || adminPass != "Password123" && attempts < 3) 
        {
            cout << "Please enter correct login information.\n";
        }
        // final check for 3 times maximum failed login
        ...
        // final else if statement for correct login credentials
